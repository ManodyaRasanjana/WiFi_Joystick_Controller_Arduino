/**
 * configure development platform as a WiFi Access Point to get data from the multiple mobile apps
 * see WiFi_Access_point and WiFi_Station examples for more details
 * 
 * MKR1000 not supported for more than one client when using WiFi_AP mode
 */

#include <WiFi_Joystick_Controller.h>

// WiFi network credentials
const char* ssid = "YOUR_AP_NAME";   // replace with desired name
const char* pswd = "YOUR_PASSWORD";  // replace with desired password
...
void setup() {
  ...
  // initialize WiFi AP and get the status. Only the first initialization should have WiFi credentials
  uint8_t wifiStatus1 = remote1.init(WJC_WIFI_MODE_AP, ssid, pswd);
  ...
  if (wifiStatus1 != WJC_ERR_OK) {
    Serial.print("Remote AP1 initialization error: ");
  ...
  if (wifiStatus2 != WJC_ERR_OK) {
    Serial.print("Remote AP2 initialization error: ");
  ...
  Serial.print("Remote AP initialized at IP Address ");
  Serial.print(remote1.getIpAddress());
  Serial.print(" with the UDP port number ");
  Serial.println(remote1.getPortNumber());
  Serial.print(" and ");
