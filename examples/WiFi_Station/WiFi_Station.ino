/**
 * configure development platform as a WiFi station to get data from the mobile app
 * make sure that mobile device and development platform connected to the same WiFi network
 */
...
void setup() {
  ...
  uint8_t wifiStatus = remote.init(WJC_WIFI_MODE_STA, ssid, pswd);
  ...
  if (wifiStatus != WJC_ERR_OK) {
    ...
    while (true) {
      // cannot continue with no WiFi connection
    }
  }
